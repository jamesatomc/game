...
class Jump9 extends FlameGame
  ...
  int level9CoinScore = 0; // Track the number of coins collected
  int level9CoinScoreReset = 0; // New variable for resetting purposes
...
// Initialize coinsText
  ...
  void resetGame() async {
    ...
    level9CoinScoreReset = 0; // Reset coin score using the new variable
  ...
  void collectCoin() {
    if (level9CoinScore < 18) {
      level9CoinScore++;
    }
    if (level9CoinScoreReset < 18) {
      level9CoinScoreReset++; // Increment the reset variable as well
    }
    coinsText.text = '$level9CoinScoreReset';
    saveLevel9CoinScore(level9CoinScore); // Save the coin score
  ...
  void updateCoinCount(int newCoinCount) {
    level9CoinScore = newCoinCount;
    coinsText.text = '$level9CoinScore';
  }

  // Example of updating the coin count
  void onCoinCollected() {
    updateCoinCount(level9CoinScore + 1);
  }

  Future<void> saveLevel9CoinScore(int score) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt('level9CoinScore', score);
  }

  Future<int?> getLevel9CoinScore() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getInt('level9CoinScore');
  }
