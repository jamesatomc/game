...
class Jump7 extends FlameGame
  ...
  int level7CoinScore = 0; // Track the number of coins collected
  int level7CoinScoreReset = 0; // New variable for resetting purposes
  ...
  Future<void> onLoad() async {
    ...
    level7CoinScore = await getLevel7CoinScore() ?? 0;
...
// Initialize coinsText
  ...
  void resetGame() async {
    ...
    level7CoinScoreReset = 0; // Reset coin score using the new variable
  ...
  void collectCoin() {
    if (level7CoinScore < 16) {
      level7CoinScore++;
    }
    if (level7CoinScoreReset < 16) {
      level7CoinScoreReset++; // Increment the reset variable as well
    }
    coinsText.text = '$level7CoinScoreReset';
    saveLevel7CoinScore(level7CoinScore); // Save the coin score
  ...
  void updateCoinCount(int newCoinCount) {
    level7CoinScore = newCoinCount;
    coinsText.text = '$level7CoinScore';
  }

  // Example of updating the coin count
  void onCoinCollected() {
    updateCoinCount(level7CoinScore + 1);
  }

  Future<void> saveLevel7CoinScore(int score) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt('level7CoinScore', score);
  }

  Future<int?> getLevel7CoinScore() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getInt('level7CoinScore');
  }

  Future<void> unlockNextLevel() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(
        'level8Unlocked', true); // Example for unlocking level 2
  }
