...
class Jump4 extends FlameGame
  ...
  int level4CoinScore = 0; // Track the number of coins collected
...
// Initialize coinsText
  ...
  void collectCoin() {
    if (level4CoinScore < 12) {
      level4CoinScore++;
    }
    if (level4CoinScoreReset < 12) {
  ...
  void onCoinCollected() {
    updateCoinCount(level4CoinScore + 1);
  }

  Future<void> saveLevel4CoinScore(int score) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt('level4CoinScore', score);
  }

  Future<int?> getLevel4CoinScore() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getInt('level4CoinScore');
  }

  Future<void> unlockNextLevel() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(
        'level5Unlocked', true); // Example for unlocking level 2
  }
