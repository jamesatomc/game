...
class Jump6 extends FlameGame
  ...
  int level6CoinScore = 0; // Track the number of coins collected
  int level6CoinScoreReset = 0; // New variable for resetting purposes
...
// Initialize coinsText
  ...
  void resetGame() async {
    ...
    level6CoinScoreReset = 0; // Reset coin score using the new variable
  ...
  void collectCoin() {
    if (level6CoinScore < 14) {
      level6CoinScore++;
    }
    if (level6CoinScoreReset < 14) {
      level6CoinScoreReset++; // Increment the reset variable as well
    }
    coinsText.text = '$level6CoinScoreReset';
    saveLevel6CoinScore(level6CoinScore); // Save the coin score
  ...
  void updateCoinCount(int newCoinCount) {
    level6CoinScore = newCoinCount;
    coinsText.text = '$level6CoinScore';
  }

  // Example of updating the coin count
  void onCoinCollected() {
    updateCoinCount(level6CoinScore + 1);
  }

  Future<void> saveLevel6CoinScore(int score) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt('level6CoinScore', score);
  }

  Future<int?> getLevel6CoinScore() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getInt('level6CoinScore');
  }

  Future<void> unlockNextLevel() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(
        'level7Unlocked', true); // Example for unlocking level 2
  }
