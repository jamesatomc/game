...
class Jump5 extends FlameGame
  ...
  int level5CoinScore = 0; // Track the number of coins collected
  int level5CoinScoreReset = 0; // New variable for resetting purposes
...
// Initialize coinsText
  ...
  void resetGame() async {
    ...
    level5CoinScoreReset = 0; // Reset coin score using the new variable
  ...
  void collectCoin() {
    if (level5CoinScore < 14) {
      level5CoinScore++;
    }
    if (level5CoinScoreReset < 14) {
      level5CoinScoreReset++; // Increment the reset variable as well
    }
    coinsText.text = '$level5CoinScoreReset';
    saveLevel5CoinScore(level5CoinScore); // Save the coin score
  ...
  void updateCoinCount(int newCoinCount) {
    level5CoinScore = newCoinCount;
    coinsText.text = ' $level5CoinScore';
  }

  // Example of updating the coin count
  void onCoinCollected() {
    updateCoinCount(level5CoinScore + 1);
  }

  Future<void> saveLevel5CoinScore(int score) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt('level5CoinScore', score);
  }

  Future<int?> getLevel5CoinScore() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getInt('level5CoinScore');
  }

  Future<void> unlockNextLevel() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(
        'level6Unlocked', true); // Example for unlocking level 2
  }
