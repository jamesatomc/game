...
class Jump8 extends FlameGame
  ...
  Future<void> onLoad() async {
    ...
    level8CoinScore = await getLevel8CoinScore() ?? 0;
...
// Initialize coinsText
  ...
  void resetGame() async {
    ...
    level8CoinScoreReset = 0; // Reset coin score using the new variable
  ...
  void collectCoin() {
    if (level8CoinScore < 16) {
      level8CoinScore++;
    }
    if (level8CoinScoreReset < 16) {
      level8CoinScoreReset++; // Increment the reset variable as well
    }
    coinsText.text = '$level8CoinScoreReset';
    saveLevel8CoinScore(level8CoinScore); // Save the coin score
  ...
  void updateCoinCount(int newCoinCount) {
    level8CoinScore = newCoinCount;
    coinsText.text = '$level8CoinScore';
  }

  // Example of updating the coin count
  void onCoinCollected() {
    updateCoinCount(level8CoinScore + 1);
  }

  Future<void> saveLevel8CoinScore(int score) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt('level8CoinScore', score);
  }

  Future<int?> getLevel8CoinScore() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getInt('level8CoinScore');
  }

  Future<void> unlockNextLevel() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(
        'level9Unlocked', true); // Example for unlocking level 2
  }
