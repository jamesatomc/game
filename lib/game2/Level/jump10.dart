...
import 'package:game_somo/game2/components/game-ui/final.dart';
...
class Jump10 extends FlameGame
  ...
  int level10CoinScore = 0; // Track the number of coins collected
  int level10CoinScoreReset = 0; // New variable for resetting purposes
...
// Initialize coinsText
  ...
  void resetGame() async {
    ...
    level10CoinScoreReset = 0; // Reset coin score using the new variable
  ...
  void collectCoin() {
    if (level10CoinScore < 18) {
      level10CoinScore++;
    }
    if (level10CoinScoreReset < 18) {
      level10CoinScoreReset++; // Increment the reset variable as well
    }
    coinsText.text = '$level10CoinScoreReset';
    saveLevel10CoinScore(level10CoinScore); // Save the coin score
  ...
  void updateCoinCount(int newCoinCount) {
    level10CoinScore = newCoinCount;
    coinsText.text = '$level10CoinScore';
  }

  // Example of updating the coin count
  void onCoinCollected() {
    updateCoinCount(level10CoinScore + 1);
  }

  Future<void> saveLevel10CoinScore(int score) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt('level10CoinScore', score);
  }

  Future<int?> getLevel10CoinScore() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getInt('level10CoinScore');
  }

  Future<void> unlockNextLevel() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(
        'level10Unlocked', true); // Example for unlocking level 2
  }
